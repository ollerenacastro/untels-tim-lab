...
| `connector-misp-feed` | **Opcional.** Eventos del feed OSINT de **CIRCL** (o cualquier feed MISP), agrupados en *reports*, con tags y con la marca `to_ids` que distingue lo accionable. Sin registro ni clave. **No arranca por defecto** — profile `misp`. | 1 h | 256 MB |
...
> **8 GB** — **7.3 GB** con el profile `otx`, y **7.6 GB** con `otx,misp` a la vez. Son
> techos, no reservas: el uso real es
...
| El lab quedó inservible y `restart-lab.sh` no lo arregla | Estado corrupto en los volúmenes, o una ingesta a medias que no drena | `./scripts/reset-lab.sh` — borra los volúmenes y vuelve a ingerir de cero. **Conserva el `.env`**, así que la contraseña de admin y las claves de feeds no cambian. Pide confirmación escrita antes de borrar |
